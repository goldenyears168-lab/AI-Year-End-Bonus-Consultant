...
3. 不要用反問的方式推進對話（不要問用戶問題）。你要從知識庫與對話內容中，主動推薦「下一個最值得問/最有價值的問題」，讓用戶一鍵接著問。
...
[給出 1 個你最推薦用戶接著問的問題（用引號包起來），並用 1 句話說明為什麼這題最有價值。不要用問號以外的追問。]
""",
