這是一份專為你的 AI 顧問系統設計的**「旗艦級」年終獎金顧問知識庫**。

這份文件的撰寫邏輯採用了 McKinsey / BCG 等頂級管顧的結構化思維，並融合了台灣中小企業（SME）的實務場景（包含勞基法觀念）。

你可以直接將這段文字複製到你的 Python 專案中的 assets/knowledge.py，作為變數 BONUS_KB_TEXT。

📂 檔案名稱：assets/knowledge.py
Python
...
"""
👨‍🏫 如何使用這份知識庫？（給系統工程師 Jack）
儲存位置：將上述內容存為 assets/knowledge.py。

RAG 整合方式：

在 nodes/advisor.py 中，直接 from assets.knowledge import BONUS_KB_TEXT。

在 System Prompt 中加入這句話：

"請參考【顧問知識庫】中的邏輯來回答。若用戶的數據觸發了【第四章：風險控管】中的條件，請務必在回答開頭加上⚠️警告符號。"

擴充性：這份文件是純文字 (Markdown)，未來如果你想增加「股權激勵」或「銷售獎金辦法」，直接往下加章節即可，完全不用改程式碼邏輯。

## 第六章：反直覺洞察與觀點化 (Counter-Intuitive Insights & Point of View)

這是頂級顧問的「靈魂」：不是給答案，而是給「觀點」。以下三條規則會讓 AI 講出老闆不敢講的話。
...
- 話術範例：「除了年終獎金，公司額外設立了『年度突破獎』，你因為 XX 專案獲選，再發 2 萬元。」
...
> 「小王，這次的年終是 0。我想很誠實地告訴你，這不是懲罰，而是公司對你未來發展的投資。你有三個選擇：第一，留職停薪 3 個月去進修，回來後我們重新評估；第二，轉到 XX 部門，那裡可能更適合你的特質；第三，如果你覺得這裡不適合，我們和平分手，我給你寫推薦信。你選哪一個？」
...
- **釋放 5% 作為「試錯基金」**：
  ...
  - 話術：「公司決定撥出 5% 盈餘作為『試錯基金』。如果你有想法，寫個提案，通過後公司給你資源，成功後你分 30% 利潤。」
...
> 「Jack，你是 S 級，公司決定給你 3 個月。這不只是獎勵，更是『市場訊號』：我們願意為頂尖人才付出市場前 10% 的代價。但相對的，C 級員工這次是 0。這不是殘忍，而是告訴所有人：『這裡只認績效，不認資歷。』」
...
- **分階段發放**：
  ...
  - 話術：「今年公司獲利不如預期，所以現在先發 0.5 個月讓大家過年。但如果 Q1 我們達成 XX 目標，4 月再發 0.5 個月。這不是畫大餅，而是『共創未來』。」
...
在 `nodes/advisor.py` 的 System Prompt 中加入：
